...
function _createSpeedButton() {
    ...
    if (_speedBtn) {
        try {
            ...
            if (typeof _get_cocos_refid === 'function') {
                stillAlive = !!_get_cocos_refid(_speedBtn);
            } else {
                // _get_cocos_refid 不存在时，用 getParent 判断
                try { stillAlive = !!_speedBtn.getParent(); } catch(e2) {}
            }
            if (stillAlive) return _speedBtn;
        } catch(e) {}
        _speedBtn = null;
        _speedLabel = null;
    }
    ...
    var step = 'start';
    try {
        // 直接用 cc.LayerColor 做容器，传入 size 防止全屏覆盖
        step = 'LayerColor.create_container';
        var btnSize = (typeof cc.size === 'function') ? cc.size(80, 80) : {width: 80, height: 80};
        var container = cc.LayerColor.create(new cc.Color(0, 0, 0, 160), 80, 80);
...
// ═══════════════════════════════════════════════════════════════
// [Keep-Alive] 每60秒静默重新应用当前倍速和动画跳过设置
// 解决场景切换后 setTimeScale 被引擎重置、hook 失效的问题
// ═══════════════════════════════════════════════════════════════
var _keepAliveTimer = setInterval(function() {
    try {
        // 重新应用当前倍速（无论是不是1x，都确保一致性）
        var curSpeed = _SPEED_LEVELS[_speedIdx];
        if (curSpeed && curSpeed > 1) {
            _applySpeed(curSpeed);
        }
        
        // 如果动画跳过已开启，重新确保 hooks 存活
        if (_animSkipEnabled) {
            _hookDelayTime();
            _hookFadeAnimations();
            try { _hookTimeSleep(); } catch(e) {}
            _hookEntityPlayAnimation(true);
            try { _hookBattleEventManager(); } catch(e) {}
        }
        // 注意：不在这里调用 _tryAttachSpeedButton()，
        // 浮窗重挂由 _director_after_draw 机制处理，
        // 避免反复创建全屏 LayerColor 导致黑屏
    } catch(e) {}
}, 60000);
